...
from pyspark.sql.functions import col, current_timestamp, lit, when as _when,sha2
...
def process_batch(batch_df: DataFrame,batch_id: int) -> None:
    if TARGET_SOURCE_PATH:
        batch_df = batch_df.filter(col('path')==TARGET_SOURCE_PATH)
    if batch_df.isEmpty():
        print(f"Batch {batch_id}: no files")
        return 
    

    batch_df = batch_df.withColumn('statement_hash',sha2(col('content'),256))
    existing_hashes = spark.table(TBL_BRONZE_EXTRACTION).select('statement_hash').distinct()
    new_files_df = batch_df.join(existing_hashes,on='statement_hash',how='left_anti')

    skipped_count = batch_df.count() - new_files_df.count()
    if skipped_count>0:
        print(f"Batch {batch_id}: skipped {skipped_count} already-known files,no extraction needed ")
    
    if new_files_df.isEmpty():
        print(f"Batc {batch_id}: nothing new to extract")
        return 
    extracted_df = new_files_df.select('path','content').mapInPandas(extract_pdfs,schema=EXTRACTION_SCHEMA)
    extracted_df = (
        extracted_df
