...
class PDFValidator:
    """
    Enterprise-grade PDF validator.

    Validation layers:
        1. Cheap Validation (byte-level checks)
        2. Deep PDF Inspection (parsed PDF checks)
    ...
    def __init__(
        ...
        if min_file_size <= 0:
            raise ValueError("min_file_size must be positive.")
        ...
        if min_file_size >= max_file_size:
            raise ValueError(
                "min_file_size must be smaller than max_file_size."
            )
        ...
        if max_page_width <= 0:
            raise ValueError("max_page_width must be positive.")

        if max_page_height <= 0:
            raise ValueError("max_page_height must be positive.")

        if max_decompression_ratio <= 0:
            raise ValueError(
                "max_decompression_ratio must be positive."
            )
        ...
        if max_xrefs <= 0:
            raise ValueError("max_xrefs must be positive.")

        for value, name in (
            (allow_forms, "allow_forms"),
            (allow_open_actions, "allow_open_actions"),
            (allow_xfa, "allow_xfa"),
        ):
        ...
        if file_size < min_file_size:
            raise PDFValidationError(
                f"File size below minimum threshold ({min_file_size} bytes)."
            )

        if file_size > max_file_size:
            raise PDFValidationError(
                f"File size exceeds maximum limit ({max_file_size} bytes)."
            )
    ...
    def _check_eof_marker(self):
        tail = self.contents[-2048:]

        if b"%%EOF" not in tail:
            raise PDFValidationError(
                "PDF EOF marker not found. File may be truncated."
            )
    ...
    def _scan_malicious_actions(self):
        for token in self.DANGEROUS_ACTIONS:
            if token in self.contents:
                raise PDFValidationError(
                    f"Dangerous PDF action detected: "
                    f"{token.decode(errors='ignore')}"
                )
    ...
    def _scan_open_actions(self):
        ...
        if b"/OpenAction" in self.contents:
            raise PDFValidationError(
                "OpenAction entries are not permitted."
            )
    ...
    def _scan_acroforms(self):
        ...
        if b"/AcroForm" in self.contents:
            raise PDFValidationError(
                "Interactive PDF forms are not permitted."
            )
    ...
    def _check_object_count(self):
        object_count = len(
            re.findall(rb"\bobj\b", self.contents)
        )

        if object_count > self.max_objects:
            raise PDFValidationError(
                f"PDF contains excessive objects ({object_count})."
            )
    ...
    def _check_page_count(self):
        ...
        if self.page_count > self.max_pages:
            raise PDFValidationError(
                f"PDF exceeds maximum page limit ({self.max_pages})."
            )
    ...
    def _check_page_dimensions(self):
        for page_number, page in enumerate(
            ...
            try:
                ...
                raise PDFValidationError(
                    f"Invalid page dimensions on page "
                    f"{page_number}: {exc}"
                ) from exc

            if (
                width > self.max_page_width
                or height > self.max_page_height
            ):
                raise PDFValidationError(
